                             ...
                             uint32_t value)
{
	...
	if (list_entry != NULL) {
		...
		error = pthread_mutex_unlock(&hash_table_entry->mutex);	// -5 points otherwise
		if (error) { 
			exit(error);
		}
		return;
	...
	error = pthread_mutex_lock(&hash_table_entry->mutex);
	if (error) { 
		exit(error);
	}
	list_head = &hash_table_entry->list_head;
